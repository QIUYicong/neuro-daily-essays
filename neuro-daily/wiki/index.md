...
- 🟢 [短时程突触可塑性（STP）](concepts/short-term-synaptic-plasticity.md) — Syt7感知残余Ca²⁺驱动STF；囊泡池耗竭驱动STD；P_r决定方向；赋予突触时间滤波、增益控制和活动静默工作记忆能力 **[NEW 2026-06-10]**
...
| 课程主题 | 状态 |
|---------|------|
| 1. 神经元如何工作 | 🔵 进行中（已覆盖：AIS, 动作电位, 突触传递, NMDA/AMPA 受体, CaMKII, 树突计算, 锥体神经元, 电压门控钙通道, PV+/SST+/VIP+ 中间神经元, 去抑制回路, LTD, **短时程突触可塑性（STP）**；待覆盖：神经调质, 突触稳态） |
...
| 4. 学习和记忆 | 🔵 进行中（已覆盖：LTP, Hebb 规则, BTSP, 海马回路, 场所场形成, SWR 重播, 印迹细胞, 记忆巩固（系统层面）, LTD, **短时程突触可塑性（STP）**；待覆盖：记忆提取机制, 突触稳态）|
