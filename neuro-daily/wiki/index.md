...
>
> 最后更新：2026-08-19（第101篇·鸡尾酒会问题：听觉景观分析的神经机制——从耳蜗到高阶颞叶皮层的声景分割）· 主题页总数：**236**（新建2页：auditory-scene-analysis、temporal-coherence；修订2页：auditory-cortex rev4、auditory-dual-stream rev2；图谱251节点、1416边）
...
- 🟡 [时间相干性（听觉）](concepts/temporal-coherence.md) — 沙玛等人2011提出的听觉流绑定理论：同步波动的神经特征来自同一声源，应被绑定为同一听觉流；注意主动提升相干性检测灵敏度参与流形成；心理物理预测力强，神经实现机制待验证（emerging） **[NEW 2026-08-19 #101]**
...
- 🟢 [听觉景观分析](systems/auditory-scene-analysis.md) — 大脑将混合声学输入组织为独立听觉流的机制；A1=全景代表，高阶STG=注意选择性放大（ECoG直接证明）；种群分离+适应性增益调节+时间相干性绑定；双流的前端基础设施；老年/APD的核心障碍点 **[NEW 2026-08-19 #101]**
- 🟢 [听觉双流架构](systems/auditory-dual-stream.md) — 腹侧流（前颞→VLPFC：声音身份→语义，双侧）vs 背侧流（后颞→Spt→Broca/运动：空间/言语运动，强左侧化）；VLSM 138例卒中直接验证（PMID:27956600）；半球谱-时序不对称（左快/右慢，96例SEEG）；背侧流独特延伸至发音运动是人类语言的皮层基础；前序基础为听觉景观分析 **[NEW 2026-08-18 #100；修订 2026-08-19]**
