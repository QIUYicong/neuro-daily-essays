...
>
> 最后更新：2026-08-19（第105篇·前额叶执行控制三功能·目标维持/干扰抑制/规则切换·回路分解）· 主题页总数：**224**（新建2页：concepts/executive-control、concepts/cognitive-flexibility；修订2页：systems/prefrontal-cortex rev4、concepts/attractor-network rev3；新增Q-dlpfc-hierarchy-mechanism；图谱232节点/1338边）
...
- 🔵 [执行控制](concepts/executive-control.md) — 目标维持/干扰抑制/规则切换三组分框架（Friedman & Robbins 2022双生子遗传学双分离）；DLPFC低维稳定子空间（非持续放电主导）；mPFC→dmStr维持期通路（Wilhelm 2023光遗传因果）；IFOF白质通路→干扰抑制；SST中间神经元正交子空间门控；楔前叶→DLPFC两步时序（约120ms）；LPFC层级梯度（尾侧具体→中部元决策→额极抽象）**[NEW 2026-08-19]**
- 🟢 [认知灵活性](concepts/cognitive-flexibility.md) — 规则切换的神经底层；腹外侧PFC损伤优先损害灵活性（非空间WM）；SST中间神经元树突抑制维持正交子空间隔离；规则×负反馈联合选择性神经元触发切换；楔前叶（注意重定向）→DLPFC（规则重映射）约120ms串行流水线 **[NEW 2026-08-19]**
...
- 🟢 [前额叶皮层（PFC）](systems/prefrontal-cortex.md) — dlPFC 第2/3/5层循环回路是工作记忆的关键脑区；PV-γ轴是认知控制的时序基础；**执行控制三功能分解（目标维持子空间/干扰抑制白质通路/规则切换SST门控）；LPFC层级梯度（rev4 2026-08-19）** **[NEW 2026-06-05]**
...
| 5. 认知控制 | 🔵 进行中（已覆盖：**工作记忆**、**奖励学习/三因素规则（2026-06-07）**、**注意/增益控制（ACh/NE，2026-06-12）**、**执行控制三功能分解（DLPFC三组分：目标维持/干扰抑制/规则切换，2026-08-19）**；待覆盖：决策深度）|
