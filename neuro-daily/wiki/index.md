...
- 🔵 [印迹细胞](concepts/engram-cells.md) — 特定经历发生时被活动依赖机制优先标记的稀疏神经元集群（DG 2–4%）；记忆的物理载体；印迹≠突触强化（沉默印迹，Ryan 2015）；填补了最高优先级悬空引用 **[NEW]**
- 🔵 [记忆分配](concepts/memory-allocation.md) — CREB 竞争决定哪些神经元成为印迹细胞；内在兴奋性竞争；~6 小时记忆链接窗口 **[NEW]**
...
| 4. 学习和记忆 | 🔵 进行中（已覆盖：LTP, Hebb 规则, BTSP, **海马回路**, 场所场形成, SWR 重播, **印迹细胞**（记忆的物质形式）；待覆盖：LTD, 记忆巩固（系统层面），电压门控钙通道）|
| 2, 5–12 | ⚪ 待开始 |

**第7篇（2026-05-30）**：**第一周综合**——归纳嵌套时间编码层级（Nested Temporal Coding Hierarchy）框架，连接前 6 篇文章的核心机制，指向第二周方向（印迹细胞、记忆巩固系统、钙通道）。

**第8篇（2026-05-31）**：**印迹细胞**——填补最高优先级悬空引用，建立"特定记忆 ↔ 特定神经元集群"的因果层，从分子层（LTP）到行为层（记忆回忆）打通细胞中间层。核心发现：印迹≠突触强化（Ryan 2015 沉默印迹）；记忆分配遵循 CREB 竞争规则；场所细胞是空间情景记忆印迹候选。
...
- ~~`engram-cells`（印迹细胞）~~ ✅ **2026-05-31 已建立**（填补了最高优先级悬空引用）
