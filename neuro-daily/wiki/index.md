...
**第143篇（2026-09-13）**：**空间转录组学——让每个细胞在原位"开口说话"**——课程脊柱第10项（方法革命）。核心：(1) MERFISH 技术原理——多轮荧光原位杂交 + 二进制纠错码实现单细胞分辨率千基因级原位检测；(2) BICCN 2023 全鼠脑图谱（Yao et al. PMID:38092916 + Zhang et al. PMID:38092912）——34类→338亚类→1,201超类→5,322集群的四级分层框架；(3) 背腹侧二元性：背侧少而多元（皮层），腹侧多而近似（下丘脑/脑干）；(4) 星形胶质细胞 36 个转录组学亚类，颠覆"胶质细胞均一"旧观；(5) 纹状体、皮层连续空间梯度。新建2个wiki页面（methods/spatial-transcriptomics、concepts/brain-cell-type-hierarchy），修订1页（systems/astrocyte rev3→rev4），图谱285→287节点、1649→1659边，新增未解问题 Q-stx-01/02/03。
