...
## 2026-07-19 · 文章 #87 · 皮肤之上的地图：体感皮层如何将触摸转化为自我感知

**核心主题**：初级体感皮层（S1）的四亚区组织、感觉人形（homunculus）与皮层放大倍率、桶状皮层模型、四类机械感受器分工、皮层地图终生可塑性（使用驱动/截肢重组/幻肢痛/技能训练）。

**新建页面（3）**：
- `systems/somatosensory-cortex.md`（初级体感皮层S1）🟢 established — 四亚区3a/3b/1/2；感觉人形；桶状皮层模型；终生地图可塑性；来源：PMID:29519481, 23195880, 19038777, 34172735
- `neurons/mechanoreceptor.md`（皮肤机械感受器）🟢 established — 四类Aβ型LTMR分工；CT纤维情感触觉；PIEZO2 MET通道；来源：PMID:23972592, 24853935
- `concepts/cortical-map-plasticity.md`（皮层地图可塑性）🟢 established — 突触LTP/LTD机制；截肢→邻区入侵；卒中后回路增强非重映射（Zeiger 2021）；来源：PMID:19038777, 34172735, 11403816, 32428706

**修订页面（1）**：
- `systems/thalamus.md` rev4 — 扩充一次视丘核小节：VPL/VPM体感中继详细描述（FA/SA类型特异性分离、桶小体barreloids、VPL→S1躯干四肢/VPM→S1面部投射分工）；related新增somatosensory-cortex/mechanoreceptor

**矛盾登记（1）**：
- C-MAP-01（open）：幻肢痛与皮层重组的因果方向：Flor 2001（面区入侵→幻肢痛）vs Makin & Flor 2020（因果可能双向/保持手区表征反而镇痛）；性质：解释框架分歧，数据层面相关性一致；需大样本纵向干预研究裁决

**新增未解问题（3）**：
- Q-s1-01（高优先级）：S1训练/截肢可塑性的细胞学底物
- Q-s1-02（中优先级）：CT纤维密度个体差异与触觉感受性
- Q-s1-03（中优先级）：Zeiger 2021"无重映射"在人类卒中中的验证

**新增悬空引用（待补）**：
- `posterior-parietal-cortex`（后顶叶皮层）— 被 somatosensory-cortex 引用，感觉-运动整合和工具使用的关键；下一个高优先级新建

**图谱**：168节点，964边（+3节点，+17边）
