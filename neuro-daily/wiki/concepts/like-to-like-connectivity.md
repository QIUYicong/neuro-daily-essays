...
"同类相连"（like-to-like connectivity）规律由 Ding et al.（2025，*Nature* 640，PMCID: PMC11981947）在 MiCrONS 功能连接组数据集上系统发现。

**核心主张**：在小鼠初级视觉皮层（V1）及相邻高级视觉区（V2、AL、RL 等），视觉朝向偏好（orientation preference）、空间频率偏好等特征相似的神经元之间，突触连接概率显著更高。

这不仅仅是物理近邻效应：控制轴突/树突的物理重叠（proximity）后，like-to-like 规律仍然显著——它是一个独立于空间距离之外的功能-结构关联。

**高阶规律**：不仅"一对一"的相似性预测连接，而且一个神经元的所有突触后细胞（postsynaptic cohort）整体上也比随机预测更相似——一个神经元"选择"功能相近的伙伴群体，而不只是随机选择。

**AI 验证**：训练于视觉分类任务的循环神经网络（RNN）在不施加任何约束的情况下，自发产生与生物数据量级相近的 like-to-like 连接结构；破坏这种结构会损害网络性能——提示 like-to-like 可能是解决视觉分类问题的一般计算策略，而非生物的"偶然选择"。
...
| 主张 | 证据/方法 | 来源 | 置信度 |
|------|----------|------|--------|
| 朝向偏好相似的神经元突触连接概率显著更高（控制物理距离后仍成立） | MiCrONS数据集：钙成像功能响应+EM突触配对，统计回归 | PMCID:PMC11981947, Ding 2025 | 高（小鼠视觉皮层） |
| like-to-like 跨层次（L2/3→L5）和跨视觉区域（V1→V2/AL）成立，包括反馈连接 | MiCrONS多区域分析 | PMCID:PMC11981947 | 中高（需更多区域验证） |
| 高阶规律：突触后细胞群体功能相似度高于一对一预测 | MiCrONS统计分析 | PMCID:PMC11981947 | 中（新发现，待复现） |
...
- [[cortical-layers]] — 规律在 Layer 2/3 和 Layer 5 之间均成立
