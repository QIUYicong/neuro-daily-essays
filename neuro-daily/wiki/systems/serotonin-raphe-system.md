...
> **一句话定义**：以脑干缝际核（主要是背缝核DRN和中缝核MRN）为起源，通过约20-30万5-HT能神经元向全脑发出体积传输信号，调控情绪基调、觉醒状态、冲动控制和神经可塑性的神经调质系统。
...
与多巴胺（奖励预测误差）、去甲肾上腺素（唤醒增益）、乙酰胆碱（注意信噪比）的相对精确功能定义不同，5-HT的功能更分散、更依赖受体亚型的局部表达模式，难以被单一词语概括。
...
**背缝核（DRN）**：
- 位于中脑-脑桥边界，是最大的5-HT神经元聚集体
- 升行投射：前额叶皮层、纹状体、杏仁核、下丘脑、海马（部分）
- 与情绪调控、认知灵活性、奖励关系最密切
...
**降行投射**：桥核/延髓缝际核 → 脊髓背角（痛觉调控）

### 传输方式：体积传输（Volume Transmission）

5-HT主要从无髓鞘轴突膨体（varicosities/boutons）释放，而非典型的突触小扣。大部分释放位点不与突触后密度（PSD）直接对齐，5-HT需通过细胞外液扩散才能到达受体。这种"弥散广播"机制：
- 影响范围广（几微米至几十微米）
- 时间常数慢（秒级）
- 与快速点对点谷氨酸/GABA传递形成互补

### 受体系统

7类14种受体：
- **5-HT1族**（1A/1B/1D/1E）：Gi偶联，cAMP↓，总体抑制
  - 1A：somatodendritic自受体 + 皮层/海马突触后
  - 1B/1D：terminal自受体（控制末梢释放量）
- **5-HT2族**（2A/2B/2C）：Gq偶联，PLC→Ca²⁺，总体兴奋
  - 2A：PFC锥体神经元顶端树突（认知调控主角）
- **5-HT3**：配体门控阳离子通道（快速EPSC，胃肠兴奋）
- **5-HT4/5/6/7**：各具特异分布和信号通路

## 关键证据

| 主张 | 证据/方法 | 来源 | 置信度 |
|------|----------|------|--------|
| DRN是前脑5-HT的主要来源 | 荧光组化+损毁实验 | Dahlström & Fuxe 1964 | 高 |
| 5-HT主要通过体积传输（非突触传递）发挥作用 | 电镜+免疫组化：多数释放位点无突触对接 | Borroto-Escuela 2021, PMID:33672070 | 高 |
| 5-HT受体共14种均已克隆 | 分子克隆+功能表达 | Barnes & Sharp 1999, PMID:10462127 | 高 |
| mPFC谷氨酸神经元→直接兴奋DRN（顶-下控制） | 顺行追踪+电生理 | Celada 2004, PMID:15309042 | 高（啮齿类） |

## 连接

- [[neuromodulator-systems]] — 四大调质系统之一
- [[norepinephrine-locus-coeruleus]] — 类似的全脑广播结构（蓝斑→全脑）
- [[acetylcholine-cortex]] — 类似的注意/认知调控调质
- [[5-ht-autoreceptor]] — 本系统的核心反馈调控机制
...
- Q-5ht-volume-transmission-specificity：体积传输如何实现区域特异性？是否存在未发现的"导向机制"使5-HT弥散到特定靶细胞？
- Q-drnmrn-functional-split：DRN和MRN对同一行为（如记忆+情绪）的相对贡献如何分工？单细胞光遗传学能否实现选择性操控？
